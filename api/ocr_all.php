<?php
// api/ocr_all.php - OCR từng trang + phân loại theo text OCR, màu ảnh và luật ưu tiên GCN/GTPL/NVTC
...
if (!is_dir($sessionTempDir)) {
    jsonResponse(['success' => false, 'error' => 'Session không tồn tại']);
}
...
$pageCount = 0;
while (file_exists($sessionTempDir . sprintf('page_%04d.png', $pageCount + 1))) $pageCount++;

if ($pageCount <= 0) {
    jsonResponse(['success' => false, 'error' => 'Không tìm thấy ảnh trang OCR trong session']);
}

// =====================================================
// 7. OCR + PHÂN TÍCH MÀU TỪNG TRANG
// =====================================================
...
for ($p = 1; $p <= $pageCount; $p++) {
    $imgFile = $sessionTempDir . sprintf('page_%04d.png', $p);
    $txtBase = $sessionTempDir . sprintf('ocr_%04d', $p);

    $pageColors[$p] = analyzeImageColorOA($imgFile);
    $raw = runTesseractOA($imgFile, $txtBase);

    $pageTexts[$p] = $raw;
    $pageSerials[$p] = detectSerialOA($raw);
}

// =====================================================
// 8. PHÂN LOẠI TOÀN FILE
// =====================================================

$allText = implode("\n\n", $pageTexts);
$fileSerial = detectSerialOA($allText);
$fileColor = mergeColorInfosOA($pageColors);
$fileMatch = detectDocTypeOA($allText, $types, $pageCount, $fileColor);

// =====================================================
// 9. KẾT QUẢ TỪNG TRANG
// =====================================================

$results = [];
for ($p = 1; $p <= $pageCount; $p++) {
    $raw = $pageTexts[$p] ?? '';
    $pageMatch = detectDocTypeOA($raw, $types, $pageCount, $pageColors[$p] ?? []);

    // Ưu tiên kết quả theo trang nếu trang đã nhận diện tự tin.
    // Chỉ dùng kết quả toàn file khi trang yếu, cùng loại, hoặc hồ sơ GCN 2-4 trang có màu/tiêu đề GCN rõ.
    $finalMatch = $pageMatch;
    $pageCode = $pageMatch['code'] ?? 'UNKNOWN';
    $fileCode = $fileMatch['code'] ?? 'UNKNOWN';
    $pageConf = (int)($pageMatch['confidence'] ?? 0);
    $fileConf = (int)($fileMatch['confidence'] ?? 0);

    if ($pageCode !== 'BLANK') {
        if ($fileConf >= 75 && ($pageConf < 55 || $pageCode === $fileCode)) {
            $finalMatch = $fileMatch;
        }

        if ($fileCode === 'GCN' && $pageCount >= 2 && $pageCount <= 4 && $fileConf >= 65 && ($pageConf < 60 || $pageCode === 'GCN')) {
            $finalMatch = $fileMatch;
        }
    }

    $docCode = $finalMatch['code'] ?? 'UNKNOWN';

    $results[] = [
        'success' => true,
        'page' => $p,

        'serial' => $pageSerials[$p] ?: $fileSerial,
        'page_serial' => $pageSerials[$p],
        ...
        'candidates' => $finalMatch['candidates'] ?? [],
        'flags' => $finalMatch['flags'] ?? [],

        'page_doc_code' => $pageMatch['code'] ?? 'UNKNOWN',
        'page_confidence' => $pageMatch['confidence'] ?? 0,
        'page_scores' => $pageMatch['scores'] ?? [],
        'page_flags' => $pageMatch['flags'] ?? [],
        'page_color_analysis' => $pageColors[$p] ?? [],

        'file_doc_code' => $fileMatch['code'] ?? 'UNKNOWN',
        'file_doc_name' => isset($types[$fileMatch['code']]) ? $types[$fileMatch['code']]['name'] : 'Không nhận dạng được',
        'file_confidence' => $fileMatch['confidence'] ?? 0,
        'file_scores' => $fileMatch['scores'] ?? [],
        'file_color_analysis' => $fileColor,

        'ocr_text' => mb_substr($raw, 0, 1000, 'UTF-8'),
    ];
}
